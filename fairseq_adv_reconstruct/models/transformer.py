...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def __init__(self, args, encoder, decoder):
        ...
        self.adv_hidden = Linear(512,2)
        self.adv_hidden_d = Linear(512,2)
    ...
    def forward(
        ...
        if self.training:
            ...
            dec = decoder_out[1]['inner_states'][-1].transpose(0,1)
            # encoder loss
            pad = src_tokens.t().eq(1)
            self.encoder.adv_targets[pad] = -100 # do not compute padding loss
            adv_h_e = self.adv_hidden(enc).float()
            src_loss = F.cross_entropy(adv_h_e.view(-1, 2), self.encoder.adv_targets.reshape(-1)) # Cross Entropy
            # decoder loss
            pad = prev_output_tokens.eq(1)
            self.decoder.adv_targets[pad] = -100 # do not compute padding loss
            adv_h_d = self.adv_hidden_d(dec).float()
            tgt_loss = F.cross_entropy(adv_h_d.view(-1, 2), self.decoder.adv_targets.reshape(-1)) # Cross Entropy
            
            # loss of P(x|x') and P(y|y')  TODO add P(x|x) and P(y|y)
            logits = F.linear(encoder_out.encoder_out, self.encoder.embed_tokens.weight).float()
            logits = F.log_softmax(logits, -1)
            loss_enc = -logits.gather(dim=-1, index=src_tokens.t().unsqueeze(-1)).squeeze(-1).mul(self.encoder.adv_targets.eq(1)).sum()
            logits = F.log_softmax(decoder_out[0].float(), -1)
            loss_dec = -logits.gather(dim=-1, index=prev_output_tokens.unsqueeze(-1)).squeeze(-1).mul(self.decoder.adv_targets.eq(1)).sum()
            decoder_out[1]['rec_loss'] = loss_enc + loss_dec
            decoder_out[1]['adv_loss'] = src_loss + tgt_loss
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.mask = nn.Parameter(self.embed_tokens.weight[3].clone().unsqueeze(0).unsqueeze(0)) # (1 * 1 * 512)
    ...
    def forward(
        ...
        if self.training:
            ...
            pad = src_tokens == pad_index
            DROP_MATRIX[pad] = 0
            self.adv_targets = DROP_MATRIX.t().clone()
        x, encoder_embedding = self.forward_embedding(src_tokens)
        if self.training:
            pos = F.dropout(self.embed_positions(src_tokens), p=self.dropout, training=self.training) * DROP_MATRIX.unsqueeze(-1) # 加入位置信息
            x = x * (1 - DROP_MATRIX.unsqueeze(-1)) + self.mask * DROP_MATRIX.unsqueeze(-1) + pos
        # else:
        #     x = x * (1 - self.src_drop)
        # B x T x C -> T x B x C
        x = x.transpose(0, 1)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def extract_features(
        ...
        """
        if self.training:
            drop_index = self.mask
            pad_index = 1
            DROP_MATRIX = torch.empty(prev_output_tokens.shape, device='cuda').fill_(self.tgt_drop).bernoulli().long()
            pad_mask = prev_output_tokens == pad_index
            DROP_MATRIX[pad_mask] = 0 # dont replace pad token
            self.adv_targets = DROP_MATRIX.clone()
            # prev_output_tokens = prev_output_tokens * (1 - DROP_MATRIX) + drop_index * DROP_MATRIX
        ...
        if self.training:
            x = x * (1 - DROP_MATRIX.unsqueeze(-1)) + self.mask * DROP_MATRIX.unsqueeze(-1)
