...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def forward(
        ...
        if self.training:
            # share parameter p(x|x’)
            # using embedding matrix to predict original word
            logits = F.linear(encoder_out.encoder_out, self.encoder.embed_tokens.weight).float()
            logits = F.log_softmax(logits, -1)
            loss_enc = -logits.gather(dim=-1, index=src_tokens.t().unsqueeze(-1)).squeeze(-1).mul(self.encoder.adv_targets).sum()
            # decoder side
            logits = F.log_softmax(decoder_out[0].float(), -1)
            loss_dec = -logits.gather(dim=-1, index=prev_output_tokens.unsqueeze(-1)).squeeze(-1).mul(self.decoder.adv_targets).sum()
