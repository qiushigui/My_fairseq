...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def get_updates(self, num_updates):
        self.num_updates = num_updates
        if num_updates < self.low: # 训练至模型拟合
            self.direction = 0
        elif num_updates < self.high: # 训练判别器
            self.direction = 0.1
        else:
            self.direction = -100  # 训练模型,反向优化判别器
    ...
    def forward(
        ...
        if self.training:
            adv_loss = self.ADV(src_tokens,prev_output_tokens, encoder_out.encoder_out, decoder_out)
            decoder_out[1]['adv_loss'] = adv_loss  * self.direction
        return decoder_out
    
    def ADV(self,src_tokens,prev_output_tokens, encoder_out, decoder_out):
        pad = src_tokens.t().eq(1)
        self.encoder.adv_targets[pad] = -100
        pad = prev_output_tokens.eq(1)
        self.decoder.adv_targets[pad] = -100
        # add gaussian noise
        enc = encoder_out + torch.randn_like(encoder_out)
        dec = self.decoder.hidden + torch.randn_like(self.decoder.hidden)
        # encoder loss
        adv_h_e = self.adv_hidden(enc).float()
        src_loss = F.cross_entropy(adv_h_e.view(-1, 2), self.encoder.adv_targets.reshape(-1), reduction='none').clamp(0.2, 5).mean()
        # decoder loss
        adv_h_d = self.adv_hidden_d(dec).float()
        tgt_loss = F.cross_entropy(adv_h_d.view(-1, 2), self.decoder.adv_targets.reshape(-1), reduction='none').clamp(0.2, 5).mean()
        # average
        adv_loss = (src_loss + tgt_loss) / 2
        return adv_loss
